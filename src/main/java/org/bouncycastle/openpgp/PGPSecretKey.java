...
public class PGPSecretKey {
     ...
     * This constructor will not add any certifications but assumes that pubKey
     * already has what is required.
     *
     ...
     * @param keyEncryptor an encryptor for the key if required (null
     * otherwise).
    ...
    /**
     * Construct a PGPSecretKey using the passed in private/public key pair and
     * binding it to the passed in id using a generated certification of
     * certificationLevel.The secret key checksum is calculated using the
     * original non-digest based checksum.
     *
     ...
     * certification.
     * @param certificationSignerBuilder the builder for generating the
     * certification.
     * @param keyEncryptor an encryptor for the key if required (null
     * otherwise).
     * @throws PGPException if there is an issue creating the secret key packet
    ...
    /**
     * Construct a PGPSecretKey sub-key using the passed in private/public key
     * pair and binding it to the master key pair. The secret key checksum is
     * calculated using the passed in checksum calculator.
     *
    ...
    /**
     * Return true if this key has an algorithm type that makes it suitable to
     * use for signing.
     * <p>
     * Note: with version 4 keys KeyFlags subpackets should also be considered
     * when present for determining the preferred use of the key.
     *
     ...
     *
     * @param decryptorFactory factory to use to generate a decryptor for the
     * passed in secretKey.
     ...
     * @param oldKeyDecryptor the current decryptor based on the current
     * password for key.
     * @param newKeyEncryptor a new encryptor based on a new password for
     * encrypting the secret key material.
     */
     ...
     * @deprecated use org.bouncycastle.gpg.SExprParser - it will also allow you
     * to verify the protection checksum if it is available.
     */
